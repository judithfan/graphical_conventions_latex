...
For each game, we randomly sampled two sets of four objects: one was designated as a \emph{repeated} while the other was a \emph{control} set (see Fig. \ref{task_stimuli}B).
In half of the pairs (N=34), the four control objects were drawn from the same cluster as repeated objects; in the other half (N = 33), they were drawn from different clusters.
...
The communication task consisted of three phases using a pre-post design (Fig. \ref{task_stimuli}C). 
During a \textit{repeated} phase, there were six blocks of trials, with each of the four \emph{repeated} objects drawn once in each block.
...
\caption{drawings become increasingly similar within pairs and different between pairs; error ribbons represent 95\% CI, dotted line represents permuted baseline. \jefan{we should include a num strokes x repetition plot here well.}}
\label{within-across}
\end{figure*}
...
A linear mixed-effects model with random slopes and intercepts for each sketcher revealed that the number of strokes in drawings of repeated objects decreased steadily as a function of repetition ($b = -0.216, ~t = -6.00, ~p < .001$), showing that pairs were increasingly able to rely upon shared knowledge to communicate effectively without expending as many strokes. 
...
In ongoing work, we are investigating the ``importance'' of each stroke in each drawing for explaining similarity to the next repetition's drawing of that object, by re-rendering the drawing without each stroke and computing the feature-based similarity to the next drawing (see Fig. \ref{within-across}D). 
