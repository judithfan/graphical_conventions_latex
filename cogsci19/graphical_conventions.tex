...
Our approach was inspired by a large literature that has documented communication dynamics during extended interaction in several modalities, including language \cite{ClarkWilkesGibbs86_ReferringCollaborative,HawkinsFrankGoodman17_ConventionFormation}, gesture \cite{goldin1996silence,fay2014creating}, and drawings \cite{garrod_foundations_2007,fay2010interactive}.
...
This analysis revealed a strong overall effect of repetition ($b = 0.231, ~t = 12.8,~p < 0.001$; see Figure \ref{recog_bis}) and a weaker but reliable interaction with group membership ($b = -0.051, ~t = -2.178, ~p = 0.032$), showing that the yoked group yoked group improved nearly at the same rate as viewers in the communication experiment had, even in the absence of viewer feedback. %1x10^{-15}
...
Here we explore this possibility by examining how the high-level visual properties of drawings changed throughout an interaction. 

We hypothesized that drawings 

Specifically, we investigated three axes along which we hypothesized that the formation of graphical conventions would impact drawings: (1) the number of strokes used; (2) the 

simpler
internally consistent
different between pairs
...
To analyze how drawings changed across repetitions, we extracted the high-level perceptual features of the drawings using a pre-trained convolutional neural network. 
...
We observed that when strokes were preserved in the later sketch, the `corresponding' strokes on the earlier were most important: lesioning them was the biggest blow to the sketch similarity.  %The color of a stroke in a sketch corresponds to the similarity of the sketch to its successor when removing that stroke from the sketch. 
