...
A key experimental design choice was the use of visual objects as the targets of reference, by contrast with the verbal labels (e.g. ``art gallery'') or audio clips used in prior work \cite{GalantucciGarrod11_ExperimentalSemiotics,fay2010interactive}. % both builds on and offers several innovative findings.
...
Finally, a major open question raised by our work concerns how people use viewer feedback when deciding what information to conserve or omit across repetitions.
One possibility is that sketchers are guided by a \emph{primacy} bias in which the initial strokes they produced early on are conserved throughout an interaction, possibly because these carry the most diagnostic information about the target.
For example, sketchers may not continue to draw the backrest of a chair if it does not help distinguish the target from distractors in context.
Another possibility is that they are guided by a \emph{recency} bias: successful viewer comprehension is attributed to the most recent strokes produced, leading these to be more strongly conserved. 
For example, if the viewer was able to correctly identify the target only after the backrest was drawn, the sketcher may continue to draw this part. 
...
Developing models of visual communication behavior that can learn from such task-related feedback is a promising approach toward distinguishing among these possibilities in future work.
