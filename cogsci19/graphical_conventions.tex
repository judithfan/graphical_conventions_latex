...
A key design choice in our visual communication paradigm was the use of images as the targets of reference, compared to the verbal concepts (e.g. ``art gallery'') or audio clips used in prior work \cite{GalantucciGarrod11_ExperimentalSemiotics}. % both builds on and offers several innovative findings.
%First, it enabled greater exploration of iconicity and abstraction.
By placing referents and messages in the same modality, this choice allowed us to initially ground different pairs in the same visual features and control the initial \emph{iconicity} of sketches -- their visual resemblance to the target they are depicting.
Although we measured patterns in visual similarity between sketches that are consistent with a trajectory of increasing `abstraction' from these initial visual features, future work should directly examine the visual similarity between drawings and their corresponding target images for a more operational measure of changes in iconicity.
