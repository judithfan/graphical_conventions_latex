...
Data from two pairs were excluded due to unusually low performance (i.e., accuracy $<$ 3 s.d. below the mean). In this and subsequent experiments, participants provided informed consent in accordance with the Stanford IRB.\footnote{All materials and data are available at: \url{https://github.com/cogtoolslab/graphical_conventions}.}
...
Following previous work that has employed DCNN models to investigate perceptual similarity \cite{FanCommon2018,peterson2018evaluating,kubilius2016deep}, for each image we extract a 4096-dimensional feature vector reflecting activations in the second fully-connected layer (i.e., \texttt{fc6}), one of the highest layers in the network.
...
%\meg{would it make sense to change this to time taken for viewer to respond? I get that the way we measured this was drawDuration, but readers may be confused by what it means to "produce" (i.e. complete) a drawing when the sketcher can be interrupted any time.}
...
We found a similar pattern of results when comparing recognition accuracy between groups:
Viewers in the original experiment identified 88\% of drawings correctly, compared to 75\% by participants in the yoked condition, $t = 6.2, ~p < 0.001$.

% \footnote{Note that response times in the yoked condition were a function of viewer performance, whereas in the communication experiment they were a function of both sketcher \emph{and} viewer performance.}
...
To compute this, for each object, we considered the set of all interactions in which that object was repeatedly drawn. 

Sep each pair of interactions 

For each pair of interactions in this set, 

we computed the similarity between a drawing made in one interaction and a drawing made in the other 

such that they refer to the same object and are drawn in the same repetition.
