...
This network had been previously trained to recognize objects in photos from the Imagenet database \cite{deng2009imagenet}.
...
Examining accuracy alone yielded similar results: the yoked group improved to a greater degree across the task (yoked: +15.8\%, shuffled: +5.6\%).
...
To understand this interaction, we examined changes in accuracy and reaction time for each group. 
While viewers in the original experiment had much higher accuracy recognizing drawings overall (original: 88\%, yoked: 75\%), %, $t = 6.2, ~p < 0.001$
\emph{improvements} in accuracy over the course of the task were similar across both groups (original: +14.5\%, yoked: +15.8\%). 
The interaction was instead driven by differential improvements in reaction time, dropping from 11s to 6s in the original experiment and from only 4.6s to 3.3s in the yoked condition.
These differences likely reflect the contribution of additional sketch production time in the original experiment.
...
A second design choice was the use of a speed bonus incentivizing participants to complete trials quickly.
What role do such time pressures play in the formation of conventions?
Recent models have emphasized the role of broader \emph{production costs} in conjunction with informativity in explaining how sketchers automatically and flexibly adjust their level of detail in communication \cite{fan2019pragmatic}. 
An intrinsic preference for less costly messages may contribute to the increasing simplification of conventions: shared social context allows agents to be equally informative with fewer strokes \cite<see>[for a formal model]{HawkinsFrankGoodman17_ConventionFormation}.
These intrinsic costs may vary across individuals, however, and our speed bonus made them explicit.
