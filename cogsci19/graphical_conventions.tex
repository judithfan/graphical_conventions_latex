...
For each pair, we randomly sampled two sets of four objects that served as contexts in the reference game: one was designated as a \emph{repeated} set while the other was a \emph{control} set (Fig.~\ref{task_stimuli}B)\footnote{In half of the pairs, the four control objects were from the same stimulus cluster as repeated objects; in the other half, they were from different clusters. This design choice was made so that generalization between different clusters could be examined in future analyses. We collapse across these groups in our current analyses.}.% (N=34 for same; N=33 for different)
