...
To test the hypothesis that later drawings were systematically sparser than earlier ones, we conducted a mixed-effects regression with random intercepts for target and sketcher.
We found that drawings got significantly simpler over repetitions (XXX; $p < .001$).
...
Specifically, we operationalized importance by re-rendering the drawing without each stroke and computing the CNN-based similarity to the successive drawing (see Fig. \ref{within-across}D).
We observed that in the earlier sketch, strokes that seemed to refer to similar object parts as the remaining strokes in the later drawing were most important: lesioning them was the biggest blow to the drawing similarity.  %The color of a stroke in a drawing corresponds to the similarity of the drawing to its successor when removing that stroke from the sketch.
