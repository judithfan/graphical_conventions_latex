...
We found a significant increase in recognition performance across both groups ($b = 0.184, ~t = 12.8, ~p < 0.001$), reflecting the general benefits of practice recognizing drawings and any inherent visual correspondences between between drawings and the target object.  %10^{-15}$
Critically, however, we also found a large and reliable interaction: yoked participants improved to a substantially greater degree than shuffled participants ($b = 0.0972, ~t = 4.89, ~p<0.001$).
In addition, the yoked group was more accurate overall at identifying the target object (yoked: 74.9\%, shuffled: 68.8\%, $~t = 3.59, ~p < 0.001$). %$0.0004
...
Observers in the shuffled conditions who were deprived of this continuity, even as the temporal order was preserved, were significantly hindered in their ability to understand drawings.
