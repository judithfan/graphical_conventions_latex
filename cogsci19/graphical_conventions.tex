...
\caption{(A) Object stimuli from ShapeNet database. (B) Each pair of participants was randomly assigned to two sets of four objects, each set from one of these categories. (C) Repeated objects were drawn eight times throughout; control objects drawn once at the beginning and end of each interaction.}
\label{task_stimuli}
\vspace{-1em}
\end{center}
\end{figure*}
...
In support of this account, a linear mixed-effects model with random slopes and intercepts for each pair of participants showed that the number of strokes in drawings of repeated objects decreased steadily as a function of repetition ($b = -0.216, ~t = -6.00, ~p < .001$; Fig. \ref{within-across}A), suggesting that pairs were increasingly able to rely upon shared knowledge to communicate efficiently.
...
Our recognition control experiments suggested that the graphical conventions discovered by different pairs were specific to each interaction.
...
We tested this prediction by computing the mean similarity between drawings of the same object from each repetition, but from different pairs of participants.
In a mixed-effects regression model including linear and quadratic terms, as well as random slopes and intercepts for object and pair, we found a small but reliable negative effect of repetition on between-game drawing similarity ($b = -1.4, ~t = -2.5$; Fig.~\ref{within-across}). %and quadratic ($b= 0.42, t = 3.9$)
...
Through a series of recognition control experiments, we demonstrated that these simpler drawings were harder for independent viewers to recognize without sharing the history of interaction with the same sketcher.
...
Our paradigm both builds on previous work in graphical convention formation \cite{garrod_foundations_2007,fay2010interactive} and offers several innovative findings.
First, it enabled greater exploration of iconicity and abstraction.
Using \emph{images} as targets of reference instead of more verbal concepts like ``art gallery'' or ``Robert de Niro'' allowed us to initially ground different pairs more strongly in the same visual features and reduce primary across-pair variance. In other words, as all pairs are provided a particular visual instance of an object, we are able to better control for individual differences in the visualization of an abstract concept, a process that was necessary in prior designs.
...
Because the drawings and the targets of referents share the same modality, future work should directly examine the similarity of drawings to their corresponding target images for a more operational measure of iconicity.
...
Our work also leaves open several hypotheses about what is being reduced over extended interaction.To begin with, there are several possibilities regarding \emph{how} people are deciding which strokes to preserve or drop on each trial.
...
A more complex yet revealing analysis would be examining whether certain semantic parts of a drawing are more diagnostic than others. We may predict that graphical conventions are formed from the removal of entire undiagnostic parts that relate to the semantic structure of the object.
An important task for future work is to distinguish between these possibilities using lesion analyses like those above and more fine-grained computational models of drawing generation that allow part-level analyses.

Our work suggests that the meaning of abstract visual depictions are increasingly determined by shared knowledge rather than their visual properties alone. Understanding the fine-grained factors that drive the formation of graphical conventions is foundational for understanding \emph{how} abstract visual depictions derive their meaning. 
