...
\meg{To do this, for each object, consider the set of all interactions in which the object appeared as a repeated object in that interaction. Now, we choose two interactions from this set and for each repetition, compute the similarity between two drawings: these drawings refer to the same given object and are drawn in the same given repetition, but come from the two different interactions we chose. Averaging this similarity value over all possible pairs of interactions gives us the mean between-interaction similarity for a given object and a given repetition.}
