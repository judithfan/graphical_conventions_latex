...
Across a series of controlled experiments, we found that participant pairs discover increasingly sparse yet effective ways of depicting objects.
...
We employed modern techniques from computer vision to characterize how the high-level visual features of drawings changed, finding that drawings of the same object became more consistent within a participant pair and divergent across participants from different pairs.
...
\ndg{results are same when using just accuracy? maybe say so and indicate why this fancy measure is a better choice? could be in footnote..}
...
\ndg{and accuracy is a better measure, since the timing wasn't under the control of the yoked group, right?}
...
The observed increase fell far outside this null distribution, $CI = [-3.21, -0.60], p < .001$, providing evidence that increasingly consistent ways of drawing each object manifested only for series of drawings produced within the same interaction.
...
\ndg{maybe help the reader by pointing out that this means the slope was unlikely in the permuted distribution, even though the values themselves weren't (the curve in E lies on top of the permutation baseline...)?}
