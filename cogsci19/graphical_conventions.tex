...
The shuffled condition of our control experiments suggested that conventions were \emph{interaction-specific} and that there may be multiple equilibria of graphical representations in our dataset. 
We made the further prediction that while early sketches may be more strongly constrained by the (shared) target images, later sketches may be more variable as pairs settle into these different equilibria.
That is, the \emph{visual features} of different pairs may diverge from one another over the course of the game.
...
We conducted a permutation test to nonparametrically test whether this observed decrease in correlation was statistically significant.
For each pair and target object, we scrambled sketch identifiers across repetitions 1000 times to generate a null distribution of $t$ values .
The observed 
