...
\subsubsection{Participants} We recruited 138 participants from Amazon Mechanical Turk, who were automatically matched to form 69 pairs. \meg{Within each pair, one participant was assigned the sketcher role and the other the viewer role. Role assignments remained the same throughout the experiment.}
...
To identify groups of visually similar chairs, we first extracted high-level visual features from 2D renderings of each objects using a deep convolutional neural network, VGG-19 \cite{simonyan2014very}.
This network had been previously optimized to categorize objects in a large subset of the Imagenet database \cite{deng2009imagenet}, containing 1.2 million natural photographs of 1000 different object classes.
This and other models from this architecture class have been shown to learn internal representations that that

predict perceptual judgments about object shape similarity
...
Each image elicits a pattern of unit activations at every layer in the model, which may be expressed as a vector in this high-dimensional feature space.

VGG-19 uses a fixed, but large number of feature dimensions to represent all images.

These 4096-dimensional feature vectors reflected VGG-19 activations to object renderings in the second fully-connected layer (i.e., \texttt{fc6}). \meg{reviewers commented that it's unclear how the images were derived to 4096 features, but I think it's pretty clear in this sentence}
...
Based on these clusters, we \meg{manually} selected two categories of visually similar objects containing eight exemplars each (Fig.~\ref{task_stimuli}A).
...
For each pair of participants, we randomly sampled two sets of four objects that served as contexts in the reference game: one was designated as a \emph{repeated} set while the other was a \emph{control} set (Fig.~\ref{task_stimuli}B)\footnote{In half of the pairs, the four control objects were from the same stimulus cluster as repeated objects; in the other half, they were from different clusters. \meg{This design choice was made so that generalization between different clusters could be examined in future analyses.} We collapse across these groups in our current analyses.}.% (N=34 for same; N=33 for different)
...
During the central \textit{repeated reference} phase, there were six blocks of \meg{four} trials, and each of the four \emph{repeated} objects appeared as the target once in each block.
In a pre-test \meg{phase} at the beginning of the experiment, and a post-test \meg{phase} at the end, both repeated and control objects appeared once as targets (in their respective contexts) in randomly interleaved order.
...
\meg{For each object, we consider all interactions in which the object appeared as a repeated object. Now, we choose two of these interactions and for each repetition, compute the similarity between two drawings, both referring to the same given object and drawn in the same given repetition, but from the two different interactions. Averaging this similarity value over all pairs of interactions gives us the mean between-interaction similarity for a given object and a given repetition.}
