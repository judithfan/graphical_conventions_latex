...
In order to measure how well pairs learned to communicate throughout the rest of their interaction, we used a measure of communicative efficiency \cite<the \emph{balanced integration score,}>{Liesefeld2018} that takes both accuracy (i.e., proportion of correct viewer responses) and response time (i.e., time taken to produce each drawing) into account. \meg{would it make sense to change this to time taken for viewer to respond? I get that the way we measured this was drawDuration, but readers may be confused by what it means to "produce" (i.e. complete) a drawing when the sketcher can be interrupted any time.}
...
To do this, for each object, we consider the set of all interactions in which the object appeared as a repeated object.
Now, for a pair of interactions in this set, we compute the similarity between the drawings from these interactions such that they refer to the same given object and are drawn in the same repetition.
