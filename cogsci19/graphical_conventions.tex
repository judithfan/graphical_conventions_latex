...
For each pair, we randomly sampled two sets of four objects that served as contexts in the reference game: one was designated as a \emph{repeated} set while the other was a \emph{control} set (see Fig. \ref{task_stimuli}B).
In half of the pairs, the four control objects were drawn from the same cluster as repeated objects; in the other half, they were drawn from different clusters (we collapse across these groups in our results). % (N=34 for same; N=33 for different)
