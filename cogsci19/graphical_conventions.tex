...
\caption{Recognition performance in control conditions; ribbons are 95\% CIs} \label{recog_bis}
\end{figure}
...
\subsubsection{Third-party observers able to accumulate interaction-specific knowledge show comparable gains}
...
However, unlike viewers in the communication game, these participants were unable to provide feedback to the sketcher, who could have used information about drawings object the viewer had selected and how quickly they were able to identify the target to modify their sketches on subsequent repetitions. 
...
As such, this control experiment provided a cleaner estimate of how strongly viewing the drawings

Consistent with prior work \cite{garrod_foundations_2007}, we anticipated that this lack of social interactivity between sketcher and viewer may have a m
...
\subsubsection{Third-party observers viewing sketches from different interactions also improve, but to lesser degree}
...
In addition to sketches drawn by different sketches growing increasingly dissimilar, we predicted that sketches would become increasingly internally consistent \emph{within} games. 
...
We found that the similarity of successive sketches \emph{increases} strongly over time ($b = 0.53, t = 4.2$; see Figure \ref{within-across}) in a mixed effects regression including linear and quadratic terms as well as random intercepts and slopes for both sketcher and target. 
As a baseline, we considered the null distribution of slopes produced by first scrambling sketches across pair for each repetition and target.
The true effect was significantly greater than this null distribution, which 
