...
How does a medium spanning such a broad range of appearances, ranging from the faithfully mimetic to the abstractly symbolic, reliably convey meaning? 
On the one hand, prior work suggests that key semantic information in a drawing, e.g., what it represents, can be derived from its visual properties alone \cite{FanCommon2018,sayim2011line}. 
On the other hand, other work has argued that socially-mediated contextual factors are crucial to make appropriate inferences about what a given drawing means \cite{goodman1976languages,gombrich1969art}.

Our paper combines these two perspectives by proposing that both visual information and social context may jointly explain how drawings derive meaning \cite{fan2018modeling}, and that a critical factor affecting the balance between the two may be the degree of shared knowledge between communicators. 
Here we explore the possibility that the accumulation of shared knowledge via extended visual communication may promote the development of simple, consistent, and effective ways to depict something, even if these \textit{ad hoc} graphical conventions are less readily apprehended by others. 

To invvestigate this, we used an online drawing-based reference game in which two players repeatedly communicate about visual objects. 
On each trial, both players were shown an array of four objects; the sketcher’s goal was to draw one of these, the target, so that the viewer could guess the target as quickly as possible. 
The game included two sets of objects: objects in one set were drawn repeatedly, while those in a control set were drawn once at the beginning and again at the end of the game. 
