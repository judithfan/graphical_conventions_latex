...
In addition, the shuffled group was less accurate overall at identifying the target object (yoked: 74.9\%, shuffled: 68.8\%, $~t = 3.59, ~p = 0.0004$).
Taken together, these results suggest that third-party observers who could remain a `fly on the wall' throughout an entire interaction were able to take advantage of this continuity to better understand which object each drawing represented, relative to third-party observers who cycled through discontinuous excerpts from several different interactions. 
...
However, we also found that the yoked group was less accurate overall at identifying the target object than viewers in the communication experiment had been (communication: 87.5\%, yoked: 74.9\%, $t = 6.18, ~p < 1x10^{-8}$).
