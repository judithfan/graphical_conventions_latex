...
To understand how people learn to communicate about visual objects across repeated visual communication, we developed a drawing-based reference game for two participants (Fig.~\ref{task_stimuli}).
...
Importantly, sketchers drew the same objects multiple times over the course of the experiment, receiving feedback about the viewer's response after each trial.
...
We tested this hypothesis by examining performance in an additional \emph{control} condition including objects that were only drawn once at the beginning (the \emph{pre}-phase) and at the end (the \emph{post}-phase).
...
To obtain reliable estimates of diagnosticity in context, we required stable contexts instead of randomly sampling different contexts for each pair of participants.
We thus collected a new sample of games using only two fixed contexts, which were counter-balanced across the \emph{repeated} and \emph{control} conditions. %, with objects in the control condition only shown at the beginning and end of the task.
This close internal replication also provided an opportunity to control for the robustness of our results to the feedback mechanism: rather than allowing the viewer to \emph{interrupt} the sketcher with an early response, we required the sketcher to click a ``done'' button before the drawing was shown to the viewer.
Otherwise, the design and stimuli were identical to the task described above (see Appendix for replications of our earlier analyses on these new data).
...
\paragraph{Participants} We recruited 138 participants from Amazon Mechanical Turk, who were grouped into 69 pairs \cite{Hawkins15_RealTimeWebExperiments}.
...
They continued through all strokes of the given drawing in this way, and then proceeded to the next drawing, annotating a total of 10 different drawings.
We recruited XXX participants from Amazon Mechanical Turk such that all YYY sketches in our dataset were annotated at least once. 
